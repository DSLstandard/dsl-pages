...
/**
 * Scraped from https://www.ascii-code.com/ and converted to CSV with
 * https://www.convertcsv.com/html-table-to-csv.htm.
 *
 * FIXME: Should have pre-processed this data and committed the processed version
 * instead of doing what I am doing now...
 */
const RAW_DATA = `\
...
/**
 * See the columns of the table from https://www.ascii-code.com/
 */
export interface AsciiCode {
  value: number
  description: string

  /* Could be unused. Like code 129 */
  symbol: string | null

  htmlName: string | null
}

function parseAsciiCodes(): AsciiCode[] {
  const outputCodes: AsciiCode[] = []

  let expectedValue = 0
  const parsedCSV = Papa.parse(RAW_DATA, { header: true })

  for (const [rowIx, row] of parsedCSV.data.entries()) {
    const raise = (msg: string) => {
      throw new Error(`Error when parsing row at index ${rowIx}, ${JSON.stringify(row)}: ${msg}`)
    }

    const getColumn = (columnName: string): string => {
      /* @ts-ignore */
      if (!(columnName in row)) {
        raise(`Expected column ${columnName} not found in row`)
      }

      /* @ts-ignore */
      const value = row[columnName]
      if (typeof value !== "string") {
        raise(`Expected column ${columnName} to be a string but got ${JSON.stringify(value)}`)
      }

      return value
    }

    const valueString = getColumn("DEC")
    const value: number = parseInt(valueString)
    const description: string = getColumn("Description")
    const symbol: string | null = getColumn("Symbol") || null
    const htmlName: string | null = getColumn("HTML Name") || null

    if (Number.isNaN(value)) {
      raise(`Expected a number for DEC column but got ${JSON.stringify(valueString)}`)
    }

    if (value !== expectedValue) {
      raise(`Expected value ${expectedValue} but got ${value}`)
    }

    outputCodes.push({ value, description, symbol, htmlName })
    expectedValue += 1
  }

  if (outputCodes.length !== 256) {
    throw new Error(`Expected 256 codes but got ${outputCodes.length}`)
  }

  return outputCodes
}

let ASCII_CODES: AsciiCode[] | null = null

/**
 * Returns the list of ASCII codes, accordingly indexed from 0-255.
 *
 * The first time this function is called, it will parse the raw data and cache
 * the result. Subsequent calls will return the cached result.
 */
export function getAsciiCodes() {
  if (ASCII_CODES === null) ASCII_CODES = parseAsciiCodes()
  return ASCII_CODES
}
